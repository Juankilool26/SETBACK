...
Hexadecimal [16-Bits]
                             ...
                             27 .globl man_menu_run
                             28 .globl man_menu_pause_run
                             29 .globl man_menu_win_run
...
Hexadecimal [16-Bits]
                             ...
                             22 .include "man/interrupcion.h.s"
                              1 ;; This file is part of SETBACK.
                              2 ;; Copyright (C) 2020 TCore (@TCore14)
                              3 ;;
                              4 ;; This program is free software: you can redistribute it and/or modify
                             ...
                             19 ;;
                             20 .globl set_int_handler
                             21 .globl man_int_load_menu_song
                             22 .globl man_int_load_ingame_song
...
Hexadecimal [16-Bits]
                             ...
                             32 
   41E4                      33 _main::
                             34    ;; Ahora utilizamos esto en vez de disble firmware porque ese trozo de line de codigo
                             35    ;; autoactualizable llama a una funcion personalizada del manager de interrupciones
   41E4 CD 2A 67      [17]   36    call set_int_handler
                             37    
                             38    
   41E7                      39 _reset:
                             40    ;; Pintamos del color de fondo toda la pantalla
   41E7 CD 3A 42      [17]   41    call cpct_limpiarPantalla_asm
                             42 
                             43    ;; Llamamos a load menu song del manager de interrupciones
   41EA CD 39 67      [17]   44    call man_int_load_menu_song
                             45 
                             46    ;; Ejecutamos el menu del juego
   41ED CD 6D 67      [17]   47    call man_menu_init   
   41F0 CD 71 67      [17]   48    call man_menu_run
                             49 
                             50    ;; Paramos la ejecucion de la musica del menu
   41F3 CD 1E 70      [17]   51    call cpct_akp_stop_asm
                             52 
                             53    ;; Llamamos a load ingame song del manager de interrupciones   
   41F6 CD 40 67      [17]   54    call man_int_load_ingame_song
                             55    
                             56    ;; Comproabamos en que partida vamos a comenzar el juego desde el principio
   41F9 CD 0C 67      [17]   57    call man_game_getNumGames_A
   41FC B7            [ 4]   58    or    a
   41FD 20 05         [12]   59    jr   nz, _main_restart_game
   41FF CD A8 5C      [17]   60    call man_game_init                        ; Iniciamos el juego desde el inicio
   4202 18 03         [12]   61    jr    loop                                ; Comenzamos bucle
                             62 
   4204                      63 _main_restart_game:
   4204 CD B6 5C      [17]   64    call man_game_restart                     ; Iniciamos el juego desde el inicio
                             65 
                             66 
                             67 
   4207                      68 loop:
                             ...
                             70 
                             71 
                             72    ;; Actualizamos el juego
   4207 CD C2 5C      [17]   73    call man_game_run
                             74 
                             75 
                             76 
                             77    ;; Comprobamos si nos hemos pasado todos los niveles
...
Hexadecimal [16-Bits]
   ...
   420A 47            [ 4]   78    ld       b, a
   420B 3E 13         [ 7]   79    ld       a, #_total_levels
   420D 90            [ 4]   80    sub      b
   420E 30 05         [12]   81    jr      nc, _continue_all_game1
   4210 CD 14 68      [17]   82    call     man_menu_win_run                  ; Ejecutamos el menu de fin de juego
   4213 18 D2         [12]   83    jr       _reset
   4215                      84 _continue_all_game1:
                             85 
                             86 
                             87 
                             88    ;; Le posibilitamos al jugador salir del juego
   4215 CD 26 68      [17]   89    call     cpct_scanKeyboard_f_asm
   4218 21 08 04      [10]   90    ld      hl, #Key_Esc
   421B CD 90 68      [17]   91    call     cpct_isKeyPressed_asm
   421E 28 0E         [12]   92    jr       z, _continue_all_game2           ; Menu de pause
   4220 CD C8 67      [17]   93    call     man_menu_pause_run               ; Return input option
   4223 B7            [ 4]   94    or       a
   4224 28 05         [12]   95    jr       z, _continue_pause_menu
   4226 CD 21 5D      [17]   96    call     man_game_next_play               ; Pasamos a la siguiente partida
   4229 18 BC         [12]   97    jr       _reset                           ; Reseteamos el juego
   422B                      98 _continue_pause_menu:
   422B CD 1A 5D      [17]   99    call     man_game_continue_level          ; Continuamos por el nivel donde estabamos
   422E                     100 _continue_all_game2:
                            101 
                            102 
                            103 
                            104    ;; Controlamos la activacion y desactivacion de la señal vsync
                            105    .rept 2
                            106       halt
                            107       halt
                            108       call cpct_waitVSYNC_asm
                            109    .endm
   422E 76            [ 4]    1       halt
   422F 76            [ 4]    2       halt
   4230 CD 16 72      [17]    3       call cpct_waitVSYNC_asm
   4233 76            [ 4]    1       halt
   4234 76            [ 4]    2       halt
   4235 CD 16 72      [17]    3       call cpct_waitVSYNC_asm
                            110 
   4238 18 CD         [12]  111    jr loop
                            112 
                            113 
                            114 
                            115 
                            ...
                            118 
                            119 
                            120 ;; Funcion para limpiar toda la pantalla
                            121 ;; INPUT
                            122 ;;      0
                            123 ;; DESTROY
                            124 ;;      ALL
...
Hexadecimal [16-Bits]
   ...
   423A                     127 cpct_limpiarPantalla_asm::
   423A 21 00 C0      [10]  128    ld    hl, #0xC000    ;; [3] HL Points to Start of Video Memory
   423D 11 01 C0      [10]  129    ld    de, #0xC001    ;; [3] DE Points to the next byte
   4240 01 00 40      [10]  130    ld    bc, #0x4000    ;; [3] BC = 16384 bytes to be copied
   4243 36 00         [10]  131    ld   (hl), #0      ;; [3] First Byte = given Colour
   4245 ED B0         [21]  132    ldir 
   4247 C9            [10]  133 ret
                            134 
                            135 
                            136 
                            ...
                            141 
                            142 
                            143 ;; Funcion para pausar el juego mediante interrupciones
                            144 ;; INPUT
                            145 ;;      A: Cantidad de interrupciones a ejecutar
                            146 ;; DESTROY
                            147 ;;      ALL
                            148 ;; RETURN
                            149 ;;      0
   4248                     150 cpct_interrupt_flow::
                            151 
   4248                     152 _pause_game:
   4248 76            [ 4]  153    halt
   4249 3D            [ 4]  154    dec     a
   424A 20 FC         [12]  155    jr     nz, _pause_game
   424C C9            [10]  156    ret
